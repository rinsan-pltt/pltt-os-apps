...
Ported from pltt-agg's routes/generate_image.py. Replaces:
  ...
  * `core.database.*`                    → backend.api.db_helpers
...
from backend.api.core.fal_helper import generate_image, upscale_image
from backend.api.core.midjourney_helper import (
    ACTION_MAP,
...
from backend.api.core.router_helper import (
    RouterUnavailable,
...
from backend.api.core.storage_helper import upload_media_to_storage, provider_safe_image_url, resize_and_store
from backend.api.core.bg_session import background_session, standalone_background_session
from backend.api.core.secrets import read_secret as _secret

logger = logging.getLogger("pltt_creative_video.generate_image")

# Neither the router call nor fal_client.subscribe (run in a worker thread via
# anyio.to_thread.run_sync) enforce their own upper bound — a hung provider or
# a stalled streaming response left generations stuck at "in_progress" forever
# (and their background_session's db connection leaked "idle in transaction"
# for the same duration). This forces a failure — caught by the existing
# `except Exception` fallback chain — instead of hanging indefinitely. Matches
# the router client's own configured ceiling (router_helper._stream_image_urls,
# timeout=300.0) — legitimate (non-hung) generations were observed taking up
# to ~185s, so a shorter outer timeout here would fail them prematurely.
...
async def run_image_generation(
    ...
    if request.project_id:
        ...
        # This app's projects are video-type; image items live alongside the
        # videos they seed (the Image→Video flow), so both types are valid.
        if project.get("type") not in ("image", "video"):
    ...
    async def _single_model_task(model_name: str) -> list:
        ...
        async with standalone_background_session(ctx) as db:
            ...
            try:
                if router_refs:
                    router_urls = await asyncio.wait_for(
                        edit_image_router(
