...
async def list_favourites(
    ...
    # "image" switches Explore to the all-generated-images listing; anything
    # else keeps the favourites-only video listing.
    ...
    items = await get_user_favourites(
        ...
        item_type=(type or "").lower() or None,
    )
    total = len(items)
...
async def create_favourite(
    ...
    if body.content_type not in ("image", "video"):
