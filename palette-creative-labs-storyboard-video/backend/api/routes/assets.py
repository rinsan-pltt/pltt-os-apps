...
async def upload_reference_asset(
    ...
    key = f"story_board_video/inputs/references/{ctx.user_id}/{content_hash}{suffix}"
    ...
    # uploads/apps/storyboard_video_maker_pltt_storyboard_video_maker/<org>/story_board_video/inputs/references/<user>/<hash>.ext
...
async def upload_brandkit_asset(
    file: UploadFile = File(...),
    ctx: PluginContext = Depends(get_ctx),
):
    """Uploads a Brand Kit asset (logo, character sheet, palette, type sample,
    motion reference, …). Mirrors upload_reference_asset but keys storage and
    dedup under the "brand_asset" type so it never mixes with Long Video
    reference images — the two lists are shown in different screens."""
    data = await file.read()
    if not data:
        raise HTTPException(status_code=400, detail="Empty file")
    if len(data) > _MAX_REFERENCE_BYTES:
    ...
    content_type = file.content_type or "application/octet-stream"
    suffix = PurePosixPath(file.filename or "").suffix or ""
    key = f"story_board_video/inputs/brandkit/{ctx.user_id}/{content_hash}{suffix}"

    existing = await find_asset_by_hash(ctx.db, ctx.user_id, content_hash, asset_type="brand_asset")
    if existing:
        ex_url = existing.get("url") or ""
        if ex_url.startswith("file://"):
            try:
    ...
    asset = await upsert_reference_asset(
        ctx.db, ctx.organization_id, ctx.user_id, url, content_hash, asset_type="brand_asset"
    )
    return {"status": "created", **asset}
...
async def list_my_assets(
    ...
    return {"assets": await get_user_assets(ctx.db, ctx.user_id, asset_type=type)}
