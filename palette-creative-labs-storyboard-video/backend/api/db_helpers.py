...
async def get_items_by_generation_id(
    db: AsyncSession, user_id: str, generation_id: str
) -> list[dict]:
    """Items for one generation batch, regardless of project — lets the
    standalone story flow poll a generation's status when its SSE event is
    missed (e.g. a multi-worker backend delivers the 'completed' event to a
    different worker than the browser's EventSource)."""
    res = await db.execute(
        select(Item)
        .where(and_(Item.user_id == user_id, Item.generation_id == generation_id))
        .order_by(Item.created_at)
    )
    return [_item_to_dict(i) for i in res.scalars().all()]
...
async def find_asset_by_hash(
    db: AsyncSession, user_id: str, content_hash: str, asset_type: Optional[str] = None
) -> Optional[dict]:
    if not content_hash:
        return None
    conds = [Asset.user_id == user_id, Asset.content_hash == content_hash]
    if asset_type:
        conds.append(Asset.type == asset_type)
    res = await db.execute(select(Asset).where(and_(*conds)))
    a = res.scalar_one_or_none()
    if not a:
        return None
    return {"id": a.id, "url": a.url, "content_hash": a.content_hash, "analysis": a.analysis}
...
async def upsert_reference_asset(
    ...
    # Scoped by type so the same image uploaded under two different roles
    # (e.g. a Long Video reference vs. a Brand Kit asset) gets its own row —
    # each list only ever shows assets uploaded for that purpose.
    existing = await find_asset_by_hash(db, user_id, content_hash, asset_type=asset_type)
    if existing:
        return existing
    a = Asset(
        id=_new_id(),
...
async def get_asset(db: AsyncSession, user_id: str, asset_id: str) -> Optional[dict]:
    res = await db.execute(
...
async def get_asset_by_url(db: AsyncSession, user_id: str, url: str) -> Optional[dict]:
    """Find a reference asset by its stored URL — lets the vision analysis be
    cached even when the caller only knows the hosted URL (e.g. a video-mode
    start frame surfacing in the Long Video references)."""
    if not url:
        return None
    res = await db.execute(
        select(Asset).where(and_(Asset.user_id == user_id, Asset.url == url)).limit(1)
    )
    a = res.scalar_one_or_none()
    if not a:
        return None
...
# ---------------------------------------------------------------------------
# Long Video (story) sessions
# ---------------------------------------------------------------------------

def _story_session_to_dict(s: StorySession) -> dict:
    return {
        "id": s.id,
...
async def upsert_story_session(
    db: AsyncSession,
    organization_id: int,
    user_id: str,
    session_id: str,
    ...
    res = await db.execute(
        select(StorySession).where(
            and_(StorySession.id == session_id, StorySession.user_id == user_id)
        )
    )
    ...
    if s is None:
        ...
        s.key_frame_id = key_frame_id
        s.data = data
        s.updated_at = _now()
    await db.commit()
    await db.refresh(s)
    return _story_session_to_dict(s)
...
async def list_story_sessions(
    ...
    """Every session for the scope (oldest first): a key frame within a
    project, or the user's standalone (project-less) sessions when
    `project_id` is None. Multiple stories can coexist on the same key frame —
    generating a new one must not hide the ones already there."""
    conds = [StorySession.user_id == user_id]
    if project_id:
        conds.append(StorySession.project_id == project_id)
        conds.append(
            StorySession.key_frame_id == key_frame_id
            if key_frame_id
            else StorySession.key_frame_id.is_(None)
        )
    else:
        conds.append(StorySession.project_id.is_(None))
    res = await db.execute(
        select(StorySession).where(and_(*conds)).order_by(StorySession.created_at)
    )
    return [_story_session_to_dict(s) for s in res.scalars().all()]


async def delete_story_session(db: AsyncSession, user_id: str, session_id: str) -> bool:
    res = await db.execute(
        select(StorySession).where(
            and_(StorySession.id == session_id, StorySession.user_id == user_id)
        )
...
async def set_asset_analysis(
    db: AsyncSession, user_id: str, asset_id: str, analysis: dict
) -> bool:
    """Stores the vision-model analysis on a reference asset (Long Video)."""
    res = await db.execute(
        select(Asset).where(and_(Asset.id == asset_id, Asset.user_id == user_id))
    )
    a = res.scalar_one_or_none()
...
async def get_user_assets(
    db: AsyncSession, user_id: str, asset_type: Optional[str] = None
) -> list[dict]:
    conds = [Asset.user_id == user_id]
    if asset_type:
        conds.append(Asset.type == asset_type)
    res = await db.execute(
        select(Asset).where(and_(*conds)).order_by(desc(Asset.created_at))
    )
    return [
        {
            "id": a.id,
...
async def get_user_favourites(
    ...
    if item_type == "image":
        # Explore's "Images" tab lists EVERY completed generated image, not
        # just favourited ones.
        stmt = select(Item).where(
            and_(Item.user_id == user_id, Item.type == "image", Item.status == "completed")
        )
    else:
        # Video tabs (All Engines / per-model) stay favourites-only and never
        # mix in image items.
        stmt = select(Item).where(
    ...
    for item in res.scalars().all():
        # Long Video per-scene images/clips stay in the db but never show in
        # Explore — only the final merged video (gen_params.source == "story",
        # not "story_scene") represents that generation there.
        if (item.gen_params or {}).get("source") == "story_scene":
            continue
...
# --- Per-user OAuth connections (social publishing) --------------------------

def _oauth_connection_to_dict(c: UserOAuthConnection) -> dict:
    return {
        "id": c.id,
        "user_id": c.user_id,
        "provider": c.provider,
...
async def get_oauth_connection(
    ...
    return _oauth_connection_to_dict(c) if c else None
...
async def upsert_oauth_connection(
    ...
    c = res.scalar_one_or_none()
    if c:
        c.access_token_enc = access_token_enc
        # A re-consent doesn't always return a fresh refresh_token (Google
        # only issues one the first time, or with prompt=consent) — keep the
        # existing one rather than blanking it out.
        if refresh_token_enc:
            c.refresh_token_enc = refresh_token_enc
        c.expires_at = expires_at
        c.scopes = scopes
        c.account_label = account_label
    else:
        c = UserOAuthConnection(
            id=_new_id(),
            organization_id=organization_id,
            user_id=user_id,
            provider=provider,
    ...
    await db.commit()
    await db.refresh(c)
    return _oauth_connection_to_dict(c)


async def delete_oauth_connection(db: AsyncSession, user_id: str, provider: str) -> bool:
    res = await db.execute(
        delete(UserOAuthConnection).where(
            and_(
                UserOAuthConnection.user_id == user_id,
                UserOAuthConnection.provider == provider,
            )
