...
from backend.api.routes.social_publish import router as social_publish_router
...
# Image generation backs the "generate images first, then animate them" flow:
# the panel's Image→Video mode posts to /generate/image and feeds the result
# into /generate/video as a start/reference frame. The chat agent's tools also
# call these handlers directly (see import-order note).
...
# Long Video mode: story planning, per-scene video prompts, ffmpeg merge.
router.include_router(story_video_router)
# Publish screen: per-user YouTube OAuth connect + Shorts upload.
