...
logger = logging.getLogger("pltt_creative_video.agent")
...
def _models_context() -> str:
    ...
    try:
        from backend.api.routes.generate_image import _EDIT_ALLOWED_MODELS as edit_capable
    ...
    for kind in _provider_order(provider):
        ...
        try:
            ...
            decision = await structured.ainvoke(
                messages, config={"run_name": "pltt-creative-video-intent-router"}
            )
            logger.info(
...
async def run_agent(
    ...
    config = {"run_name": "pltt-creative-video-agent", "metadata": metadata or {}}
