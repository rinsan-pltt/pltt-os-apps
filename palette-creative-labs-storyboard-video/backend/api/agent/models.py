...
These live in their own module — NOT in `backend.api.models` — deliberately.
The hosted runtime is a long-lived process that caches modules in
`sys.modules` across plugin revisions: `backend.api.models` is routinely a
STALE module object from a pre-chat revision, so `from backend.api.models
...
`backend.api.models` re-exports these classes on first load for fresh
...
class AgentThread(OrgScopedTable):
    ...
    __tablename__ = "pltt_storyboard_video_maker__agent_threads"
...
class AgentMessage(OrgScopedTable):
    __tablename__ = "pltt_storyboard_video_maker__agent_messages"

    id: Mapped[int] = mapped_column(_BigPk, primary_key=True, autoincrement=True)
    thread_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("pltt_storyboard_video_maker__agent_threads.id", ondelete="CASCADE"),
        index=True,
    )
...
class AgentThreadImage(OrgScopedTable):
    ...
    __tablename__ = "pltt_storyboard_video_maker__agent_thread_images"
