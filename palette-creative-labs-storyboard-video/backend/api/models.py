...
with `pltt_storyboard_video_maker__` per the plugin schema convention.
...
'pltt_storyboard_video_maker__projects' is already defined for this MetaData instance."
...
if _other is not None:
    ...
    class Project(OrgScopedTable):
        __tablename__ = "pltt_storyboard_video_maker__projects"
    ...
    class KeyFrame(OrgScopedTable):
        __tablename__ = "pltt_storyboard_video_maker__key_frames"

        id: Mapped[str] = mapped_column(String(64), primary_key=True, default=_uuid)
        project_id: Mapped[str] = mapped_column(
            String(64),
            ForeignKey("pltt_storyboard_video_maker__projects.id", ondelete="CASCADE"),
            index=True,
        )
    ...
    class KeyFrameAsset(OrgScopedTable):
        __tablename__ = "pltt_storyboard_video_maker__key_frame_assets"

        id: Mapped[str] = mapped_column(String(64), primary_key=True, default=_uuid)
        key_frame_id: Mapped[str] = mapped_column(
            String(64),
            ForeignKey("pltt_storyboard_video_maker__key_frames.id", ondelete="CASCADE"),
            index=True,
        )
    ...
    class Item(OrgScopedTable):
        ...
        __tablename__ = "pltt_storyboard_video_maker__items"
    ...
    class Asset(OrgScopedTable):
        ...
        __tablename__ = "pltt_storyboard_video_maker__assets"
        ...
        # Vision-model analysis for the Long Video mode: {kind, name,
        # description}, kept with the image so the story planner can cast the
        # reference into scenes and content-hash dedup reuses the analysis.
    ...
    class StorySession(OrgScopedTable):
        """Long Video (story) session snapshot: the full flow state (story,
        scenes with prompts/generation ids/urls, references, step, models) so
        a page refresh restores the board and its inputs. Scoped per key frame
        within a project — a story shows only on the key frame it was created
        on; both NULL for sessions planned from the standalone page."""

        __tablename__ = "pltt_storyboard_video_maker__story_sessions"
    ...
    class UserOAuthConnection(OrgScopedTable):
        """A user's OAuth connection to a third-party publish destination
        (currently "youtube"; "instagram" to follow the same shape later).
        One row per (user_id, provider) — a fresh connect overwrites it.
        Access/refresh tokens are stored Fernet-encrypted (see
        backend/api/core/oauth_crypto.py); plaintext tokens never touch the
        database or logs."""

        __tablename__ = "pltt_storyboard_video_maker__oauth_connections"
        __table_args__ = (
        ...
        # Provider account identity (e.g. channel title), shown in the UI so
        # the user knows which account is connected.
