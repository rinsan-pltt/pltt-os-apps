...
/** Same multipart flow as uploadReferenceImage, but for Brand Kit assets
 * (logos, character sheets, palettes, …) — kept in a separate backend "type"
 * so they never mix with Long Video reference images. */
export async function uploadBrandKitAsset(file: File): Promise<UploadResult> {
  const form = new FormData()
  form.append("file", file, file.name)
  return (await apiRequest("/assets/brandkit", {
    method: "POST",
    body: form,
  })) as UploadResult
}
