"""Initial schema for the Pltt Creative plugin.
...
def upgrade() -> None:
    ...
    ensure_org_rls(op, "pltt_creative__projects")

    op.create_table(
        "pltt_creative__key_frames",
        sa.Column("id", sa.String(64), primary_key=True),
        sa.Column("organization_id", sa.BigInteger(), nullable=False, index=True),
        sa.Column(
            "project_id",
            sa.String(64),
            sa.ForeignKey("pltt_creative__projects.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
    ...
    ensure_org_rls(op, "pltt_creative__key_frames")

    op.create_table(
        "pltt_creative__key_frame_assets",
        sa.Column("id", sa.String(64), primary_key=True),
        sa.Column("organization_id", sa.BigInteger(), nullable=False, index=True),
        sa.Column(
            "key_frame_id",
            sa.String(64),
            sa.ForeignKey("pltt_creative__key_frames.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
    ...
    ensure_org_rls(op, "pltt_creative__key_frame_assets")
    ...
    ensure_org_rls(op, "pltt_creative__items")

    op.create_table(
        "pltt_creative__assets",
        sa.Column("id", sa.String(64), primary_key=True),
        sa.Column("organization_id", sa.BigInteger(), nullable=False, index=True),
        sa.Column("user_id", sa.String(64), nullable=False, index=True),
        sa.Column("url", sa.Text(), nullable=False),
    ...
    op.create_index(
        "ix_pltt_creative__assets_user_content_hash",
        "pltt_creative__assets",
        ["organization_id", "user_id", "content_hash"],
        unique=True,
        postgresql_where=sa.text("content_hash IS NOT NULL"),
    )
    ensure_org_rls(op, "pltt_creative__assets")


def downgrade() -> None:
    op.drop_index("ix_pltt_creative__assets_user_content_hash", table_name="pltt_creative__assets")
    op.drop_table("pltt_creative__assets")
    op.drop_table("pltt_creative__items")
    op.drop_table("pltt_creative__key_frame_assets")
    op.drop_table("pltt_creative__key_frames")
    op.drop_table("pltt_creative__projects")
