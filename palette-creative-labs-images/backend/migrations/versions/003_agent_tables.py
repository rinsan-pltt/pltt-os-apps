...
def upgrade() -> None:
    ...
    ensure_org_rls(op, "pltt_creative__agent_threads")

    op.create_table(
        "pltt_creative__agent_messages",
        sa.Column("id", _BigPk, primary_key=True, autoincrement=True),
        sa.Column("organization_id", sa.BigInteger(), nullable=False, index=True),
        sa.Column(
            "thread_id",
            sa.String(64),
            sa.ForeignKey("pltt_creative__agent_threads.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
    ...
    ensure_org_rls(op, "pltt_creative__agent_messages")

    op.create_table(
        "pltt_creative__agent_thread_images",
        sa.Column("id", _BigPk, primary_key=True, autoincrement=True),
        sa.Column("organization_id", sa.BigInteger(), nullable=False, index=True),
        sa.Column(
            "thread_id",
    ...
    ensure_org_rls(op, "pltt_creative__agent_thread_images")


def downgrade() -> None:
    op.drop_table("pltt_creative__agent_thread_images")
    op.drop_table("pltt_creative__agent_messages")
    op.drop_table("pltt_creative__agent_threads")
