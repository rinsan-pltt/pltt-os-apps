...
logger = logging.getLogger("pltt_creative.talk")
...
class ShareGenerationRequest(BaseModel):
    channel: str = Field(..., min_length=1)
    # Optional lead-in line, e.g. "@Priya here's the hero shot".
...
async def post_to_talk(ctx: PluginContext, text: str, channel: str) -> Optional[dict]:
    """Fire-and-forget Talk post for background work (generation callbacks,
...
async def share_generation_to_talk(
    ...
    Talk takes plain text, so the image travels as its URL — clients unfurl it.
    """
    ...
    if not url:
        raise HTTPException(
            status_code=409,
            detail="That generation has no image yet — wait for it to finish.",
        )
    ...
    if model:
        lines.append(f"— generated in Image Studio with {model}")
