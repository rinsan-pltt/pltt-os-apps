"""Database helper functions for the Pltt Creative plugin.
...
from pltt_image_backend.api.models import (
    Asset,
    Item,
    KeyFrame,
    KeyFrameAsset,
...
async def get_user_favourites(
    ...
    stmt = select(Item).where(and_(Item.user_id == user_id, Item.is_favourite.is_(True)))
    if group_is_null:
