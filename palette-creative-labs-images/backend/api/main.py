"""Pltt Creative plugin backend entry.

The Palette platform automatically mounts this `router` at
`/api/v1/plugins/pltt-creative/*`. No FastAPI app instance, no uvicorn, no
...
# the fully-qualified `pltt_image_backend.api.X` name. This avoids two failure modes
...
#     `pltt_image_backend.api.models`) would register each OrgScopedTable on the
...
# sys.path (which is what makes `from pltt_image_backend.api.X` work), so without this the
...
# in `sys.modules` across plugins. Both this app and the sibling video app
# expose the SAME package path `pltt_image_backend.api.*` (e.g. `pltt_image_backend.api.models`), so a
# plain `import pltt_image_backend.api.models` can return the OTHER app's already-cached
# copy — which defines the video app's `pltt_creative_video__*` tables and makes
# every query hit a relation that doesn't exist in THIS plugin's schema. Evict
# any cached `backend`/`backend.*` (and generic-alias) modules whose file is NOT
...
# DEFINITIVE isolation: load THIS plugin's package under the globally unique
# name `pltt_image_backend`, bound to this file's own directory by explicit
# path. Eviction alone proved insufficient in the hosted process — after
# evicting the sibling video app's cached `backend.*`, the re-import of
# `backend` resolves through sys.path, whose ORDER the host loader (not this
# file) controls, and the video app's root can win. A spec_from_file_location
# import under a name no other plugin uses takes sys.path out of the equation
# entirely: `pltt_image_backend.api.*` submodules can only ever come from this
# plugin's own tree.
import importlib.util as _ilu

_UNIQUE_PKG = "pltt_image_backend"
...
if _cached_pkg is None or not _cached_file.startswith(_root_str):
    # Drop any copy from a previous revision (extracted to a different dir)
    # so every submodule below reloads from THIS revision's files.
    for _name in [
        n for n in list(sys.modules)
        if n == _UNIQUE_PKG or n.startswith(_UNIQUE_PKG + ".")
    ]:
        del sys.modules[_name]
...
# the `pltt_image_backend.api.routes` package. The hosted reviewer process keeps modules
# cached in `sys.modules` across plugin revisions: when a previous revision's
# `pltt_image_backend.api.routes` package is still cached, `from pltt_image_backend.api.routes
...
try:
    ...
    logging.getLogger("pltt_creative").error(
        "chat router unavailable, /agent routes disabled:\n%s", _chat_import_error
    )

router = PluginRouter(tags=["pltt-creative"])
...
# Video generation moved to the dedicated "Pltt Creative Video" plugin —
# this image-only app no longer exposes /generate/video.
...
async def chat_diagnostics(ctx: PluginContext = Depends(get_ctx)):
    ...
    import pltt_image_backend.api.routes as _routes_pkg
    ...
    if chat_router is not None:
        import pltt_image_backend.api.routes.chat as _chat_mod
