...
logger = logging.getLogger("pltt_creative.storage")

_FOLDER = {
    "images": "creative_images/outputs/images",
    "videos": "creative_images/outputs/videos",
    "masks": "creative_images/inputs/masks",
}
...
async def upload_media_to_storage(ctx, source_url: str, media_type: str) -> str:
    ...
    `creative_images/outputs/<media>/<uuid>.<ext>`. Returns the canonical file URL.
    ...
    folder = _FOLDER.get(media_type, f"creative_images/outputs/{media_type}")
...
async def upload_data_url_to_storage(ctx, data_url: str, media_type: str) -> str:
    ...
    browser) to app storage under `creative_images/outputs/<media>/<uuid>.<ext>`
    (or `creative_images/inputs/masks/<uuid>.<ext>` for masks) and returns the
    canonical file URL.
...
def _unlink_local(file_url: str) -> None:
    ...
    try:
        ...
        # Best-effort cleanup of empty parent dirs (e.g. the generations/<id>
