/**
 * Centralized API client for the Pltt Creative plugin.
 *
 * All plugin backend routes live under `/api/v1/plugins/pltt-creative/*`.
 *
...
const BASE = "/api/v1/plugins/pltt-creative"
...
// `https://…/api/v1/appstore/previews/<id>/plugins/pltt-creative` plus a
...
// `/api/v1/plugins/pltt-creative/events/…` path, which 404s in a preview.
  ...
  _originProbe = (async () => {
    try {
      ...
      if (resolved) {
        ...
        // scoping like `/appstore/previews/<id>/plugins/pltt-creative`).
        _apiBase = {
          origin: u.origin,
