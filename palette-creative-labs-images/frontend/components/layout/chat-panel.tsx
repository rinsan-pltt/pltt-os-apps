...
// A saved-but-not-yet-sent mask is staged per conversation scope (see
// ScopeChat.editMask*) — persisted here too so it survives a refresh instead
// of only living in React state.
const storedMaskKey = (scope: string) => `palette:chatMask:${scope}`

type StoredMask = { editMask: string; editMaskData: string; editMaskImageUrl: string }

const readStoredMask = (scope: string): StoredMask | null => {
  try {
    const raw = localStorage.getItem(storedMaskKey(scope))
...
const writeStoredMask = (scope: string, data: StoredMask) => {
  try {
    localStorage.setItem(storedMaskKey(scope), JSON.stringify(data))
  } catch {
    /* storage unavailable — the mask just won't survive a refresh */
  }
}

const clearStoredMask = (scope: string) => {
  try {
    localStorage.removeItem(storedMaskKey(scope))
  } catch {
    /* storage unavailable — nothing to clear */
  }
}
...
export const ChatPanel = () => {
  ...
  React.useEffect(() => {
    ...
    // A mask saved before the refresh (but not yet sent) only lives in
    // localStorage at this point — restore it into the scope alongside the
    // thread fetch below so it's back before the user can even notice.
    const storedMask = readStoredMask(scope)
    patchScope(scope, { loading: true, ...(storedMask ?? {}) })
      ; (async () => {
        try {
