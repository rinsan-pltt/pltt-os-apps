...
function AppThemeRoot({
  ...
  React.useLayoutEffect(() => {
    ...
    const probe = () => {
      ...
      // `parent` is host-chrome DOM, not ours — the OS shell can reconcile
      // its own children around the same resize event that triggers this
      // probe. If it does, our insert/measure/remove races the host's own
      // commit and either DOM call can throw ("removeChild: node is not a
      // child of this node"), which without this guard propagates as an
      // uncaught error and trips the on-screen crash reporter. Losing the
      // race just means this one measurement is skipped — fillViewport
      // keeps its last known-good value, so nothing user-visible breaks.
      try {
        parent.appendChild(probeEl)
        const percentHeightResolves = probeEl.offsetHeight > 0
        setFillViewport(!percentHeightResolves)
      } catch (err) {
        console.warn("[pltt-creative-video] viewport probe skipped (host DOM race)", err)
      } finally {
        if (probeEl.parentNode === parent) {
          try {
            parent.removeChild(probeEl)
          } catch {
            // Already detached by the host — nothing left to clean up.
          }
        }
      }
    }
    ...
    // screen. Coalesced onto one rAF per resize burst (a drag-resize fires
    // dozens of `resize` events) — this both avoids redundant reflows and
    // narrows the window where the host's own re-layout of `parent` can
    // land mid-probe (see the try/catch above for when it does anyway).
    let raf = 0
    const onResize = () => {
  ...
  return (
    ...
    >
      <PlttCreativeVideoPortalProvider container={portalContainer}>
        ...
        <LayoutErrorBoundary label="AppShell">
          <AppShell>
      ...
      `}</style>
        <div
          ...
          data-pltt-creative-video-portal-root
          // Mirror the active theme onto the portal root so the --pltt* tokens
          // declared for `[data-pltt-creative-video-portal-root]` in globals.css
          // resolve correctly for portaled dialogs/popovers. In the scoped
          // published/OS build the `.dark` class theme does NOT reach this
          // portaled subtree by inheritance, so the tokens are re-declared here
          // and selected by this attribute instead. (Dark is the CSS default;
          // this attribute drives the light override.)
          data-pltt-creative-video-theme={theme}
          style={{
            position: "absolute",
            inset: 0,
