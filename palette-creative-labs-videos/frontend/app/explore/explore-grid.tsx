          ...
          {col.map((fav) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "relative overflow-hidden rounded-lg border border-border40 group cursor-pointer",
                  isSelected && "ring-1 ring-foreground"
                )}
              >
                <HoverVideo src={fav.url} poster={fav.thumbnail_url} duration={fav.duration} className="w-full h-auto" />
                {compareMode && (
                  <span
                    className={cn(
