...
import { VIDEO_ENGINE_PILLS, MARK_COLORS, type EngineFilter, type Favourite, type MarkFilter, type GridMode } from "@/app/explore/types"
...
function VideoCard({ item, onClick }: { item: Item; onClick: () => void }) {
  return (
    <div
      ...
      onClick={onClick}
    >
      <HoverVideo src={item.url!} type="video" poster={item.thumbnail_url} className="w-full h-auto" />
      {item.group && (
        <div
...
function KeyFrameSection({
  ...
  const completed = (keyFrame.items ?? []).filter((i) => i.status === "completed" && i.url && (i.type ?? "video") === "video")
...
export function ProjectArchivePage({ projectId }: { projectId: string }) {
  ...
  const totals = React.useMemo(() => {
    ...
    const videos = all.filter((i) => (i.type ?? "video") === "video")
    const completedOf = (items: Item[]) => items.filter((i) => i.status === "completed" && i.url).length
    return {
      keyFrames: kfs.length,
      videoCompleted: completedOf(videos),
    ...
    <div className="flex flex-col flex-1 overflow-auto p-8 gap-6">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs uppercase tracking-widest text-muted-foreground">
          ...
          <span className="text-foreground font-semibold">{totals.videoCompleted}</span>/{totals.videoTotal} VID
        </p>
      </div>
        ...
        <ImageDetailModal
          ...
          {...(() => {
            const navFavs: Favourite[] = filteredKeyFrames.flatMap((kf) =>
              (kf.items ?? [])
                .filter((item) => item.status === "completed" && item.url && (item.type ?? "video") === "video")
                .map((item) => ({
                  id: item.id,
                  type: "video",
                  url: item.url!,
