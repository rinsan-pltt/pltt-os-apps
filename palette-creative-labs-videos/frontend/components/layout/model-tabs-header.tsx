    ...
    {models.map((model) => {
      ...
      return (
        <button
          ...
          className={cn(
            "flex-1 flex flex-col items-start px-3 py-2.5 text-left transition-colors",
            // Each model is its own bordered box; collapse the shared inner
            // edges with a negative left margin so the row reads as a clean
            // strip of boxes rather than doubled 2px borders.
            "border border-border40 -ml-px first:ml-0 border-b-2",
            scrollable ? "whitespace-nowrap" : "min-w-0",
            isSelected
              ? "bg-secondary border-b-current relative z-10"
              : "border-b-border40 hover:bg-secondary/50"
          )}
        >
