...
// A group of videos generated together inside a key frame ("Scene N",
// renameable). Selecting one scopes the workspace to just that run's clips.
...
export interface ProjectInfo {
  ...
  type: "video"
  key_frames?: KeyFrame[]
  // Video panel inputs shared across the whole project — the same reference
  // images / frames follow the user between key frames.
  video_refs?: KeyFrameVideoRefs | null
  created_at?: string
  updated_at?: string
}
...
interface ProjectContextValue {
  ...
  setCurrentProject: React.Dispatch<React.SetStateAction<ProjectInfo | null>>
  ...
  // Selected scene inside the active key frame (null = key-frame view, which
  // shows the generated images; a scene shows only that scene's videos).
...
const ACTIVE_SCENE_STORAGE_KEY = "palette:activeSceneId"
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const [activeSceneId, setActiveSceneIdState] = React.useState<string | null>(() => {
    if (typeof window === "undefined") return null
    try {
      return window.sessionStorage.getItem(ACTIVE_SCENE_STORAGE_KEY)
    } catch {
      return null
    }
  })

  const setActiveSceneId = React.useCallback((id: string | null) => {
    setActiveSceneIdState(id)
    if (typeof window === "undefined") return
    try {
      if (id) window.sessionStorage.setItem(ACTIVE_SCENE_STORAGE_KEY, id)
      else window.sessionStorage.removeItem(ACTIVE_SCENE_STORAGE_KEY)
    } catch {
  ...
  const setActiveKeyFrameId = React.useCallback((id: string | null) => {
    ...
    // A key-frame selection always lands on the key-frame view; the caller
    // sets the scene afterwards when a scene row was what got clicked.
  ...
  // Drop a stale scene selection: the restored/persisted scene must belong to
  // the active key frame of the loaded project, otherwise fall back to the
  // key-frame view.
  React.useEffect(() => {
    if (!activeSceneId || !currentProject?.key_frames) return
    const kf = currentProject.key_frames.find((k) => k.id === activeKeyFrameId)
    if (!kf || !(kf.scenes ?? []).some((s) => s.id === activeSceneId)) {
      setActiveSceneId(null)
    }
  }, [activeSceneId, activeKeyFrameId, currentProject, setActiveSceneId])
