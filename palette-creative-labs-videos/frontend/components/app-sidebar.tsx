...
import { useProjectContext, type KeyFrame, type Scene } from "@/components/providers/project-context"
...
// Collapse/expand wrapper for a key frame's scene sub-list — the CSS-grid
// 0fr→1fr height trick (same as the control panel's AnimatedReveal): the
// list grows/fades open when its key frame is selected and collapses shut
// when another one is, staying mounted until the exit transition finishes.
function AnimatedSceneList({ show, children }: { show: boolean; children: React.ReactNode }) {
...
// A scene row under its key frame: click opens just that scene's videos in
// the workspace, double-click renames in place (same contract as key frames).
function SceneRow({
  scene,
  projectId,
  keyFrameId,
  ...
  isActive: boolean
  onSelect: () => void
  onRenamed: (updated: Scene) => void
  onDeleted: (id: string) => void
}) {
  const { t } = useT()
  const [editing, setEditing] = React.useState(false)
  const [draft, setDraft] = React.useState(scene.scene_name)
  const inputRef = React.useRef<HTMLInputElement>(null)
  // Deleting is destructive (the scene and its clips leave the sidebar), so
  // the X only opens a confirmation dialog — same contract as key frames.
  const [confirmOpen, setConfirmOpen] = React.useState(false)
  const [deleting, setDeleting] = React.useState(false)
  ...
  React.useEffect(() => setDraft(scene.scene_name), [scene.scene_name])

  const commit = async () => {
    const name = draft.trim()
    setEditing(false)
    if (!name || name === scene.scene_name) {
      setDraft(scene.scene_name)
      return
    }
    try {
      const res = await apiRequest(
        `/projects/${projectId}/key-frames/${keyFrameId}/scenes/${scene.id}`,
        { method: "PATCH", body: JSON.stringify({ name }) },
      )
      onRenamed({ ...scene, ...res, scene_name: res.scene_name ?? name })
    } catch (err) {
      setDraft(scene.scene_name)
      toast.error(t("errors.renameFailed"), {
        description: err instanceof Error ? err.message : String(err),
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(t("errors.deleteSceneFailed"), {
        description: err instanceof Error ? err.message : String(err),
      })
    } finally {
      setDeleting(false)
  ...
  return (
    <div
      className={cn(
        // Nested one level deeper than its key frame row (ml-12 vs ml-4) with
        // breathing room on both sides so the sub-item reads as a child, not
        // a row hugging the sidebar border.
        "group/scene flex items-center pl-3 pr-2 py-1.5 ml-12 mr-2 text-[11px] tracking-widest uppercase border-l-2 select-none transition-colors",
        isActive
          ? "border-l-primary/50 bg-secondary font-semibold"
          : "border-l-transparent hover:border-l-border text-muted-foreground hover:text-foreground hover:bg-secondary",
      )}
    >
      <IconVideo className="size-3 shrink-0 mr-1.5 opacity-60" strokeWidth={1.5} />
      {editing ? (
        <input
          ref={inputRef}
          value={draft}
          ...
          className="bg-transparent outline-none w-full text-[11px] tracking-widest uppercase"
        />
      ) : (
        <button
          type="button"
      ...
      <button
        ...
        disabled={deleting}
        className="ml-1 shrink-0 opacity-0 group-hover/scene:opacity-100 transition-opacity text-muted-foreground/60 hover:text-destructive disabled:opacity-30"
        aria-label={t("sidebar.deleteScene")}
      >
        <IconX className="size-3" strokeWidth={1.5} />
      </button>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="uppercase tracking-widest text-sm">{t("sidebar.deleteScene")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("sidebar.deleteSceneConfirm", { name: scene.scene_name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting}>{t("common.cancel")}</AlertDialogCancel>
...
function CurrentProjectSection() {
  ...
  const handleSceneDeleted = (kfId: string, sceneId: string) => {
    setCurrentProject({
      ...currentProject,
      key_frames: keyFrames.map((kf) =>
        kf.id === kfId
          ? { ...kf, scenes: (kf.scenes ?? []).filter((s) => s.id !== sceneId) }
          : kf,
      ),
    })
    // Deleting the open scene falls back to the key-frame view.
    if (activeSceneId === sceneId) setActiveSceneId(null)
  }

  const handleSceneRenamed = (kfId: string, updated: Scene) => {
    setCurrentProject({
      ...currentProject,
      key_frames: keyFrames.map((kf) =>
        kf.id === kfId
    ...
    <SidebarGroup className="p-0 py-3 border-b border-border group-data-[collapsible=icon]:hidden">
      ...
      >
        ...
        <div className="size-7 bg-secondary border border-border shrink-0 overflow-hidden flex items-center justify-center">
          {thumbnail ? (
            <video src={thumbnail} muted playsInline preload="metadata" className="size-full object-cover" />
          ) : (
          ...
          {keyFrames.map((kf, index) => (
            <React.Fragment key={kf.id}>
              <KeyFrameRow
                keyFrame={kf}
                projectId={projectId}
                isActive={activeKeyFrameId === kf.id && !activeSceneId}
              ...
              />
              {/* Scenes of the open key frame — one row per video-generation
                  run, sliding open/shut as keyframes are switched. Hidden
                  while dragging so the reorder math stays row-based. */}
              <AnimatedSceneList
                show={activeKeyFrameId === kf.id && !drag && (kf.scenes ?? []).length > 0}
              >
                {(kf.scenes ?? []).map((scene) => (
                  <SceneRow
                    key={scene.id}
                    ...
                    onRenamed={(s) => handleSceneRenamed(kf.id, s)}
                    onDeleted={(sceneId) => handleSceneDeleted(kf.id, sceneId)}
                  />
                ))}
              </AnimatedSceneList>
            </React.Fragment>
