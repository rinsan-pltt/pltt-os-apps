...
from .video_thumbnail import extract_first_frame, find_ffmpeg

logger = logging.getLogger("pltt_creative_video.storage")

_FOLDER = {
    "images": "creative_videos/outputs/images",
    "videos": "creative_videos/outputs/videos",
    "masks": "creative_videos/outputs/masks",
}
...
async def upload_media_to_storage(ctx, source_url: str, media_type: str) -> str:
    ...
    `creative_videos/outputs/<media>/<uuid>.<ext>`. Returns the canonical file URL.
    ...
    folder = _FOLDER.get(media_type, f"creative_videos/outputs/{media_type}")
...
async def upload_data_url_to_storage(ctx, data_url: str, media_type: str) -> str:
    ...
    browser) to app storage under `creative_videos/outputs/<media>/<uuid>.<ext>`
    and returns the canonical file URL.
...
def _unlink_local(file_url: str) -> None:
    ...
    try:
        ...
        # Best-effort cleanup of empty parent dirs (e.g. the
        # creative_videos/outputs/<media> folder when this was its only file).
        parent = path.parent
