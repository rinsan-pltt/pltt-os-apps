...
then return zero rows *silently* and writes blow up with
    ...
    for table "pltt_creative_video__projects"

which is exactly what `POST /projects` hit on dev-os / qa-os / os.
...
session over) runs with no org context at all.
...
_LISTENER_ATTR = "_pltt_creative_video_org_guc"
